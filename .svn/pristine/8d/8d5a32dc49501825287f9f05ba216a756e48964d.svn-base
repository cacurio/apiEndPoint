package com.websystique.springmvc.model;

import java.sql.Timestamp;

public class ModelServicioRentasInternas {

	private String fechaCancelacion;
	private String estadoContribuyente;
	private String fechaInicioActividades;
	private String listaBlanca;
	private String fechaReinicioActividades;
	private String fechaSuspensionDefinitiva;
	private String numeroRuc;
	private String tipoContribuyente;
	private String actividadEconomica;
	private String estadoSociedad;
	private String claseContribuyente;
	private String razonSocial;
	private String personaSociedad;
	private String fechaActualizacion;
	private String obligado;
	private String estadoPersona;

	public ModelServicioRentasInternas() {
	}

	public String getFechaCancelacion() {
		return fechaCancelacion;
	}

	public void setFechaCancelacion(String fechaCancelacion) {
		this.fechaCancelacion = fechaCancelacion;
	}

	public String getEstadoContribuyente() {
		return estadoContribuyente;
	}

	public void setEstadoContribuyente(String estadoContribuyente) {
		this.estadoContribuyente = estadoContribuyente;
	}

	public String getFechaInicioActividades() {
		return fechaInicioActividades;
	}

	public void setFechaInicioActividades(String fechaInicioActividades) {
		this.fechaInicioActividades = fechaInicioActividades;
	}

	public String getListaBlanca() {
		return listaBlanca;
	}

	public void setListaBlanca(String listaBlanca) {
		this.listaBlanca = listaBlanca;
	}

	public String getFechaReinicioActividades() {
		return fechaReinicioActividades;
	}

	public void setFechaReinicioActividades(String fechaReinicioActividades) {
		this.fechaReinicioActividades = fechaReinicioActividades;
	}

	public String getFechaSuspensionDefinitiva() {
		return fechaSuspensionDefinitiva;
	}

	public void setFechaSuspensionDefinitiva(String fechaSuspensionDefinitiva) {
		this.fechaSuspensionDefinitiva = fechaSuspensionDefinitiva;
	}

	public String getNumeroRuc() {
		return numeroRuc;
	}

	public void setNumeroRuc(String numeroRuc) {
		this.numeroRuc = numeroRuc;
	}

	public String getTipoContribuyente() {
		return tipoContribuyente;
	}

	public void setTipoContribuyente(String tipoContribuyente) {
		this.tipoContribuyente = tipoContribuyente;
	}

	public String getActividadEconomica() {
		return actividadEconomica;
	}

	public void setActividadEconomica(String actividadEconomica) {
		this.actividadEconomica = actividadEconomica;
	}

	public String getEstadoSociedad() {
		return estadoSociedad;
	}

	public void setEstadoSociedad(String estadoSociedad) {
		this.estadoSociedad = estadoSociedad;
	}

	public String getClaseContribuyente() {
		return claseContribuyente;
	}

	public void setClaseContribuyente(String claseContribuyente) {
		this.claseContribuyente = claseContribuyente;
	}

	public String getRazonSocial() {
		return razonSocial;
	}

	public void setRazonSocial(String razonSocial) {
		this.razonSocial = razonSocial;
	}

	public String getPersonaSociedad() {
		return personaSociedad;
	}

	public void setPersonaSociedad(String personaSociedad) {
		this.personaSociedad = personaSociedad;
	}

	public String getFechaActualizacion() {
		return fechaActualizacion;
	}

	public void setFechaActualizacion(String fechaActualizacion) {
		this.fechaActualizacion = fechaActualizacion;
	}

	public String getObligado() {
		return obligado;
	}

	public void setObligado(String obligado) {
		this.obligado = obligado;
	}

	public String getEstadoPersona() {
		return estadoPersona;
	}

	public void setEstadoPersona(String estadoPersona) {
		this.estadoPersona = estadoPersona;
	}

	@Override
	public String toString() {
		return "ModelServicioRentasInternas [fechaCancelacion=" + fechaCancelacion + ", estadoContribuyente="
				+ estadoContribuyente + ", fechaInicioActividades=" + fechaInicioActividades + ", listaBlanca="
				+ listaBlanca + ", fechaReinicioActividades=" + fechaReinicioActividades
				+ ", fechaSuspensionDefinitiva=" + fechaSuspensionDefinitiva + ", numeroRuc=" + numeroRuc
				+ ", tipoContribuyente=" + tipoContribuyente + ", actividadEconomica=" + actividadEconomica
				+ ", estadoSociedad=" + estadoSociedad + ", claseContribuyente=" + claseContribuyente + ", razonSocial="
				+ razonSocial + ", personaSociedad=" + personaSociedad + ", fechaActualizacion=" + fechaActualizacion
				+ ", obligado=" + obligado + ", estadoPersona=" + estadoPersona + "]";
	}
	
	

}
